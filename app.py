...
# requests is optional - for external API calls
try:
    import requests
...
def index():
    ...
    # Ensure vulnerability_count exists on all entries (backward compat)
    for scan in user_scans:
        if 'vulnerability_count' not in scan:
            vulns = scan.get('vulnerabilities', [])
            scan['vulnerability_count'] = len(vulns) if vulns else 0
    
    recent_scans = user_scans[-5:] if user_scans else []
    
    # Calculate stats
    total_scans = len(user_scans)
    total_vulns = sum(s.get('vulnerability_count', 0) for s in user_scans)
    critical_count = sum(1 for s in user_scans if s.get('vulnerability_count', 0) >= 5)
...
@app.route('/landing')
def landing():
    """Marketing landing page - no login required"""
    return render_template('landing.html')

@app.route('/godmode')
def godmode():
    """GODMODE cinematic landing page - no login required"""
    return render_template('godmode.html')

@app.route('/login')
def login():
    """Login page - renders GODMODE template"""
...
def auth_google():
    ...
    if not OAUTH_AVAILABLE:
        flash('OAuth not available. Install authlib: pip install authlib', 'error')
        return redirect(url_for('login'))
...
def history():
    ...
    return render_template('history.html', scans=user_history, role=role)

@app.route('/admin/login', methods=['GET', 'POST'])
def admin_login():
    """Secret admin login page with extra security"""
    if request.method == 'POST':
        username = request.form.get('username', '').strip()
        password = request.form.get('password', '').strip()
        admin_key = request.form.get('admin_key', '').strip()
        
        # Verify admin key (set in .env or use default for demo)
        master_key = os.getenv('ADMIN_SECRET_KEY', 'CYBERPUNK_ADMIN_2024')
        ...
        # Check password (handle pbkdf2, sha256, or plain)
        ...
        elif len(stored_password) == 64:  # SHA256 hash
            import hashlib
            password_valid = (stored_password == hashlib.sha256(password.encode()).hexdigest())
        else:
        ...
        if not user.get('active', True):
            flash('Admin account deactivated', 'error')
            return render_template('admin_login.html')
        ...
        session['admin_access'] = True  # Special flag for admin portal
        
        logger.warning(f"Admin login successful: {username} from {request.remote_addr}")
...
@admin_required
def admin_dashboard():
    """Admin dashboard with suspicious activity monitoring"""
    users = load_users()
    history_data = load_scan_history()
    ...
    for username, user_data in users.items():
        reasons = []
        user_scans = [s for s in history_data if s.get('scanned_by') == username]
        scan_count = len(user_scans)
        
        # Check for suspicious patterns
        if scan_count > 50:  # Too many scans
            reasons.append("EXCESSIVE_SCANNING")
        
        if user_data.get('failed_logins', 0) > 5:  # Multiple failed logins
            reasons.append("FAILED_LOGIN_ATTEMPTS")
        
        # Check for rapid scanning (more than 10 scans in 1 hour)
        recent_scans = [s for s in user_scans if s.get('timestamp')]
        if len(recent_scans) > 10:
            reasons.append("RAPID_SCANNING")
        
        # Check for suspicious target patterns
        suspicious_targets = ['localhost', '127.0.0.1', '0.0.0.0', 'internal', 'admin', 'api']
        for scan in user_scans:
            target = scan.get('target', '').lower()
            if any(s in target for s in suspicious_targets):
                reasons.append("SUSPICIOUS_TARGET")
                break
        
        # Check for multiple accounts from same email
        email = user_data.get('email', '')
        if email:
            same_email_users = [u for u, d in users.items() if d.get('email') == email and u != username]
            if len(same_email_users) > 2:
                reasons.append("MULTIPLE_ACCOUNTS")
        
        # Check for inactive but suddenly active (simplified)
        if user_data.get('last_login') and user_data.get('created_at'):
            # Skip complex date parsing - just check if account is old and active
            pass
        ...
        if reasons:
            ...
            activity_logs.append({
                ...
                'message': f'User "{username}" flagged: {", ".join(reasons)}',
                'level': 'danger'
            })
    
    # Add recent system logs
    activity_logs.extend([
        {'time': datetime.now().strftime('%H:%M:%S'), 'message': 'System security scan completed', 'level': 'info'},
        {'time': '20:30:15', 'message': 'New user registration: user123', 'level': 'info'},
        {'time': '20:15:22', 'message': 'Failed login attempt from IP 192.168.1.100', 'level': 'warning'},
    ])
...
@app.route('/admin/toggle-user/<username>')
@admin_required
def admin_toggle_user_get(username):
    """Toggle user status (GET route for admin panel)"""
    ...
    logger.warning(f"Admin {session.get('username')} {status} user: {username}")
    flash(f'User "{username}" {status}', 'success')
    return redirect(url_for('admin_users'))

@app.route('/admin/delete-user/<username>')
@admin_required
def admin_delete_user_get(username):
    """Delete user (GET route for admin panel)"""
    if username == session.get('username'):
    ...
    logger.warning(f"Admin {session.get('username')} deleted user: {username}")
...
def api_start_scan():
    ...
    def run_scan():
        ...
        try:
            ...
            # Always generate the HTML report regardless of scan source
...
<body>
    ...
    <script>
        const vulnerabilityData = {vulnerability_json};
        const scanMetadata = {{
            scan_id: '{scan_id}',
            target_url: '{target_url}',
...
def view_report(scan_id):
    """Open HTML report - checks multiple possible locations"""
    ...
    # Check scan history for custom report path
    history = load_scan_history()
    for scan in history:
        if scan.get('scan_id') == scan_id:
            # Try the path from history
            custom_path = scan.get('report_path', '')
            if custom_path and os.path.exists(custom_path):
                return send_file(custom_path)
            # Try with just the filename
            if custom_path:
                filename = os.path.basename(custom_path)
                alt_path = os.path.join(REPORTS_DIR, filename)
                if os.path.exists(alt_path):
                    return send_file(alt_path)
            break
    
    # Try with UUID pattern (some reports use UUID)
    for filename in os.listdir(REPORTS_DIR):
        if filename.endswith('.html'):
            # Check if this report matches by reading its content
            try:
...
def download_report(scan_id):
    """Download HTML report - checks multiple possible locations"""
    # Try direct path first
    report_path = os.path.join(REPORTS_DIR, f"report_{scan_id}.html")
...
if __name__ == '__main__':
    ...
    print(f"OAuth: {'Available' if OAUTH_AVAILABLE else 'Not Available (install authlib)'}")
